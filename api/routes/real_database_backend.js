// Real Backend Server with PostgreSQL 18 Database Connection
...
// PostgreSQL 18 Database Connection
const pool = new Pool({
...
app.get('/api/health', async (req, res) => {
  try {
    const dbTest = await queryDatabase('SELECT COUNT(*) as table_count FROM information_schema.tables WHERE table_schema = $1', ['public']);
    const tableCount = dbTest.rows[0].table_count;
    
    res.json({
      status: 'healthy',
      database: 'PostgreSQL 18',
      timestamp: new Date().toISOString(),
      server: 'Express.js with Real Database',
      tables: tableCount
    });
  } catch (error) {
    ...
    res.json({
      status: 'healthy',
      database: 'PostgreSQL 18 (Connection Error)',
      timestamp: new Date().toISOString(),
      server: 'Express.js with Real Database',
      error: error.message
    });
...
app.get('/api/studies', authenticateToken, async (req, res) => {
  try {
    ...
    try {
      // Try to get from real database with joins
      result = await queryDatabase(`
...
// Start server
app.listen(PORT, () => {
  console.log(`Real Database Backend Server running on port ${PORT}`);
