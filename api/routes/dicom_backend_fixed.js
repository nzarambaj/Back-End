// Fixed DICOM Backend Service
// Complete DICOM processing with proper async/await
...
// Get DICOM metadata
app.get('/api/dicom/metadata/:filename', authenticateToken, async (req, res) => {
  try {
...
// Enhance DICOM image
app.post('/api/dicom/enhance/:filename', authenticateToken, async (req, res) => {
  try {
    const { filename } = req.params;
...
app.get('/api/dicom/statistics/:filename', authenticateToken, async (req, res) => {
  try {
    const { filename } = req.params;
    const filePath = path.join(dicomDir, filename);
