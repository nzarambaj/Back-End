#!/usr/bin/env python3
"""
Test DICOM Backend Service
Test all DICOM processing functionality
"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for file_info in files[:3]:  # Show first 3
                    ...
                    print(f"        Thumbnail: {'Yes' if file_info['thumbnail_available'] else 'No'}")
    ...
    def test_upload_dicom(self):
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(f"   Original: {file_info.get('original_filename', '')}")
                print(f"   Stored: {file_info.get('stored_filename', '')}")
                print(f"   Size: {file_info.get('file_size', 0)} bytes")
                print(f"   Thumbnail: {'Yes' if file_info.get('thumbnail_path') else 'No'}")
                print(f"   PNG: {'Yes' if file_info.get('png_path') else 'No'}")
                ...
                return file_info.get('stored_filename')
            else:
                print(f"   Status: FAILED - HTTP {response.status_code}")
    ...
    def test_enhance_image(self, filename):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Status: SUCCESS")
                print(f"   Enhancement: {result.get('enhancement_type', '')}")
                print(f"   Image Size: {len(result.get('enhanced_image', ''))} chars")
                return True
            else:
                print(f"   Status: FAILED - HTTP {response.status_code}")
                return False
    ...
    def test_statistics(self, filename):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Mean Value: {stats.get('mean_value', '')}")
                print(f"   Std Value: {stats.get('std_value', '')}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\n TROUBLESHOOTING:")
        print("1. Check DICOM backend service is running")
